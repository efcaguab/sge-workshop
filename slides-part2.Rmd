...
That's why we need a queuing system...
...
- Ensures that jobs are contained and the nodes are shielded from possible errors

- Ensures that jobs are run only within allocated resources (cores/RAM memory/time)

##

The master node hosts the queuing system. That's why we shouldn't run jobs there.

##

If the master node crashes, the queuing system crashes too. Which means everyone's jobs might go with it. No one can use the cluster until the whole thing is reset. 

# SGE

##

There are multiple queuing systems available. Abacus uses SGE (Sun Grid Engine System). 
...
try submitting a job and then getting info about it
